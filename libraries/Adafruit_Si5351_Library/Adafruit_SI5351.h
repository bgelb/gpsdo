...
public:
  ...
  si5351_err_t begin(TwoWire *theWire = &Wire); //!< @return ERROR_NONE
  si5351_err_t setClockBuilderData(void);       //!< @return ERROR_NONE
  si5351_err_t setupPLL(si5351PLL_t pll, uint8_t mult, uint32_t num,
                 uint32_t denom);                   //!< @return ERROR_NONE
  si5351_err_t setupPLLInt(si5351PLL_t pll, uint8_t mult); //!< @return ERROR_NONE
  si5351_err_t setupMultisynth(uint8_t output, si5351PLL_t pllSource, uint32_t div,
                        uint32_t num, uint32_t denom); //!< @return ERROR_NONE
  si5351_err_t setupMultisynthInt(uint8_t output, si5351PLL_t pllSource,
                           si5351MultisynthDiv_t div); //!< @return ERROR_NONE

  si5351_err_t enableSpreadSpectrum(bool enabled);
  si5351_err_t enableOutputs(bool enabled);
  /*!
   * @param output Enables or disables output
   * @param div Set of output divider values (2^n, n=1..7)
   */
  si5351_err_t setupRdiv(uint8_t output, si5351RDiv_t div); //!< @return ERROR_NONE

private:
  si5351Config_t m_si5351Config;

  Adafruit_I2CDevice *i2c_dev = NULL; ///< Pointer to I2C bus interface
  si5351_err_t write8(uint8_t reg, uint8_t value);
  si5351_err_t read8(uint8_t reg, uint8_t *value);
  si5351_err_t writeN(uint8_t *data, uint8_t n);
